...
namespace mm
{
    ...
    std::wstring ClipLas::stringToWString(const std::string& str)
    {
    ...
    std::string ClipLas::wstringToString(const std::wstring& wstr)
    {
    ...
    void ClipLas::ListFilesInDirectory(const std::string& directoryPath, std::vector<std::string>& outFiles)
    {
    ...
    void ClipLas::ListFilesInDirectory(const std::string& directoryPath)
    {
        std::wstring searchPath = stringToWString(directoryPath) + L"\\*";
        WIN32_FIND_DATA findData;
        HANDLE hFind = FindFirstFile(searchPath.c_str(), &findData);
        ...
        if (hFind != INVALID_HANDLE_VALUE)
        {
            do
            {
                if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
                {
                    this->m_lasFiles.push_back(wstringToString(findData.cFileName)); 
                }
            } while (FindNextFile(hFind, &findData) != 0);
            FindClose(hFind);
        }
    ...
    void ClipLas::setOutputLasPath(std::string lasOutputFilePath)
    {
        this->m_clippedLasFileDir = lasOutputFilePath;
    }
    ...
    bool ClipLas::getAllLasFiles()
    {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(this->m_lasFileDirName))
        {
            ...
            this->m_lasFiles.push_back(entry.path().string());  
        }
