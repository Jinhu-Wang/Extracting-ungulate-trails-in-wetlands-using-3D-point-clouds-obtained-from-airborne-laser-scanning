...
#include "../PointCloud/BasicStructure.h"
...
namespace ms
{
    ...
    private:
        ...
        kdTree *m_nKDTree;
        PointCloud2<double> m_nKdPts;
        bool m_nIsKDTreeBuilt;
    ...
    public:
        ...
        MarchingSquares(PointCloud *cloud,
                        const Limits &x_limits,
                        const Limits &y_limits,
                        const Resolution &resolution);

        //
        MarchingSquares(const Limits &x_limits,
                        const Limits &y_limits,
                        const Resolution &resolution,
                        const PointCloud *cloud);

        // give only the points that are considered;
        MarchingSquares(const Limits &x_limits,
                        const Limits &y_limits,
                        const Resolution &resolution,
                        const std::vector<dPoint3D> points);

        std::vector<std::array<Node, 4>> m_Grids;

        EdgeList compute_ms(double iso_value) const;
    };
